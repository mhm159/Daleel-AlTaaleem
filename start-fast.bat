...
echo.
echo [2/4] Starting servers in Fast Development Mode (Turbopack)...
start "LGS-Backend-Fast" cmd /k "cd /d ""%BACKEND%"" && npm run dev"
timeout /t 3 /nobreak >nul

start "LGS-Frontend-Fast" cmd /k "cd /d ""%FRONTEND%"" && npm run dev"
...
taskkill /fi "WINDOWTITLE eq LGS-Backend-Fast*" /f >nul 2>&1
taskkill /fi "WINDOWTITLE eq LGS-Frontend-Fast*" /f >nul 2>&1
